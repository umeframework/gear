...
// 创建配置对象
func NewConfigIni(file string) *ConfigIni {
    cfg := new(ConfigIni)
...
// 返回段落集合
func (this *ConfigIni) ParagraphSet() []string {
    var keys []string
...
// 返回段落内容
func (this *ConfigIni) Paragraph(paragraph string) map[string]string {
    _,exist := this.paragraphs[paragraph]
    if exist {
...
// 读取文本值
func (this *ConfigIni) Get(paragraph string, key string) string {
    return this.paragraphs[paragraph][key]
}

// 设置文本值（暂不支持文件保存）
func (this *ConfigIni) Set(paragraph string, key string, value string) {
    this.sync.Lock()
    defer this.sync.Unlock()
    _,exist := this.paragraphs[paragraph]
    if !exist {
...
// 移除（暂不支持文件保存）
func (this *ConfigIni) Remove(paragraph string, key string) {
    this.sync.Lock()
    defer this.sync.Unlock()
    _,exist := this.paragraphs[paragraph]
    if exist {
...
// 读取文本值列表
func (this *ConfigIni) GetList(paragraph string, key string) []string {
...
// 读取整数值
func (this *ConfigIni) GetInt(paragraph string, key string) (int, error) {
    s := this.Get(paragraph, key)
    return strconv.Atoi(s)
}

// 读取整数值列表
func (this *ConfigIni) GetIntList(paragraph string, key string) []int {
